...
| `gemini : O termo 'gemini' não é reconhecido como nome de cmdlet` (ou `'gemini' não é reconhecido como um comando interno ou externo`) | O `npm install -g` funcionou, mas a pasta global do npm não está no PATH desta sessão — quase sempre porque o terminal já estava aberto antes da instalação. Não é instalação corrompida | **Fechar e reabrir o terminal** e rodar `gemini --version`. Se ainda falhar: `npm list -g --depth=0` (tem que listar `@google/gemini-cli`); descobrir a pasta com `npm config get prefix` e conferir que ela está no **Path** das Variáveis de Ambiente do Windows. Atalho pra destravar na hora: `npx @google/gemini-cli`. ⚠️ O pacote é `@google/gemini-cli` — `@google/generative-ai-cli` **não existe** e devolve `npm error 404` |
| `usage limit reached for gemini` / `Usage limit reached for gemini-3.5-flash` no meio do setup | Limite **gratuito diário** da conta Google usada no Gemini CLI. Não é erro de instalação, não é problema do produto e **não** tem relação com a `GEMINI_API_KEY` do agente | Três saídas, o usuário escolhe: (1) esperar o reset da cota e continuar — o setup é retomável, não recomeça do zero; (2) abrir `codex` ou `claude` **na mesma pasta** e digitar `INICIAR SETUP` (é o mesmo setup); (3) usar outra conta Google no Gemini CLI. **Nunca** mandar comprar crédito de API pra conseguir instalar |
